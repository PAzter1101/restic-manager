...
class ResticService:
    ...
    def get_snapshots(
        self, host: Optional[str] = None, tag: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        if result.returncode != 0:
            raise HTTPException(
                status_code=500, detail=f"Restic error: {result.stderr}"
            )

        snapshots: List[Dict[str, Any]] = json.loads(result.stdout)
    ...
    def get_snapshot_size(self, snapshot_id: str) -> int:
        ...
        try:
            ...
            if stats_result.returncode == 0:
                stats: Dict[str, Any] = json.loads(stats_result.stdout)
    ...
    def get_snapshot_files(self, snapshot_id: str) -> List[Dict[str, Any]]:
    ...
    def create_backup(
        self, files: List[UploadFile], tags: Optional[List[str]] = None
    ) -> Dict[str, str]:
            ...
            if file_size > settings.max_file_size_bytes:
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        f"Max size: {settings.MAX_FILE_SIZE}MB"
                    ),
                )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for line in lines:
                if "snapshot" in line.lower() and "saved" in line.lower():
                    ...
                    for part in parts:
                        if (
                            len(part) == 8 and part.isalnum()
                        ):  # ID снапшота обычно 8 символов
                            snapshot_info["snapshot_id"] = part
                            break

            snapshot_info["message"] = "Backup created successfully"
            snapshot_info["files_count"] = str(len(files))
